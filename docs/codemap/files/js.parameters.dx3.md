...
| 行 | 名前 | シグネチャ | 行数 |
|---:|---|---|---:|
| 77 | renderDX3CharacterPanel | `renderDX3CharacterPanel({ container, mode, canEdit = true, parameters, components, onComponentChange, getComponents, getToken, getEffectiveParameterValue, dispatch, tokenId, allowParameterEdit = false })` | 4 |
...
| 530 | handleDX3ChatCommand | `handleDX3ChatCommand(rawInput, { token, dispatch, getEffectiveParameterValue, generateBuffId, rollBCDice })` | 1 |
| 604 | resetDX3ComponentsOnPhaseEnd | `resetDX3ComponentsOnPhaseEnd(components, phase)` | 16 |
| 630 | renderDX3BuffFields | `renderDX3BuffFields({ container, paramId })` | 1 |
