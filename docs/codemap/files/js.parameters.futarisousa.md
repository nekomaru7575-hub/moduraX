...
| 881 | const | FUTARISOUSA_PLUGIN | `FUTARISOUSA_PLUGIN` |  |

## トップレベル関数（LOCAL TASKS 候補）（29）
...
| 431 | sheetNumber | `sheetNumber(value)` | 6 |  |
| 454 | importSkillsFromSheet | `importSkillsFromSheet(json)` | 9 |  |
| 465 | importStressFromSheet | `importStressFromSheet(json)` | 4 |  |
| 471 | sheetCost | `sheetCost(value)` | 4 |  |
| 476 | importActionsFromSheet | `importActionsFromSheet(json)` | 13 |  |
| 490 | importGuestsFromSheet | `importGuestsFromSheet(json)` | 13 |  |
| 507 | importEmotionsFromSheet | `importEmotionsFromSheet(json)` | 14 |  |
| 526 | importFutariSousaParameters | `importFutariSousaParameters(json, charType)` | 18 |  |
| 551 | importFutariSousaCharacterJson | `importFutariSousaCharacterJson(json)` | 28 |  |
| 580 | buildTypeSelect | `buildTypeSelect(charType)` | 11 |  |
| 592 | renderFutariSousaCharacterPanel | `renderFutariSousaCharacterPanel({ container, mode, canEdit = true, parameters = {}, components, onComponentChange, getComponents, dispatch, getToken, generateBuffId, findTokenByName, tokenId })` | **288** |  |
