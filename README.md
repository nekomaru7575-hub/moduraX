...
| シーンと情報 | シーンの遷移、公開先を選べる「情報」。本文の一部の語だけを伏せることもできる |
| ラウンド進行 | 参加者選択、状態バー、プロット（未公開の値は他人に配らない） |
| 音楽 | BGM・効果音・システム音。発言に反応して鳴らす「再生フレーズ」 |
| スタンプ | 押すとその場で送れる浮動パネル。部屋ごとに画像を登録し、押された数を集計できる |
| キャラシ取り込み | 外部ツール（ゆとシート系など）のJSONやURLからコマを作る |
| コマ作成ツール | 部屋を作らずにコマを作る独立ページ。作ったコマはブラウザに残り、部屋へ持ち込める |
| PWA | ホーム画面・デスクトップへインストールできる |

対応済みのプラグイン: ダブルクロス3rd / シノビガミ / 銀剣のステラナイツ /
アリアンロッドRPG 2E / ドラクルージュ / フタリソウサ / グランクレスト戦記RPG
...
**スタンプとトランプの絵は、リポジトリに入っていません。** フリー素材やファンキットには
「自分のサイトで使うのはよいが素材として再配布するのは駄目」というものがあり、clone した
人全員へ配ることになるからです。無ければ無いものとして動きます（スタンプは一覧に出ず、
カードは文字で描かれます）。自分の置き場から配るなら `.env` の `ASSET_BASE_URL` に
指してください（[.env.example](.env.example) に置き方を書いてあります）。
...
| `image/` | アイコンなどの画像 |
| `test/` | `node --test` で走る自動テスト |
| `tools/` | コードマップ生成などの開発用スクリプト（アプリの動作には要りません） |
...
自動テストは、画面を持たない部分（状態遷移・外部JSONの取り込み・上限の判定など）に
だけあります。`npm test` で走ります。画面まわりは手で確かめる領域なので、その手順は
[docs/plugin-guide.md](docs/plugin-guide.md) の「動作確認のしかた」にあります。
