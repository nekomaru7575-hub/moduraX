...
//
//   node .loop/codemap.mjs                        全ファイル再生成
//   node .loop/codemap.mjs --files js/main.js     指定ファイルのノートだけ更新（INDEX は常に全体）
//   node .loop/codemap.mjs --check                書かずに、散文が要更新のファイルだけ報告
//   node .loop/codemap.mjs --accept js/main.js    散文を書き直した印を付ける（prose_sha を更新）
//   node .loop/codemap.mjs --sync-vault           生成後に Obsidian Vault へコピー
//
...
const DEFAULT_VAULT = 'C:/Users/necom/OneDrive/Documents/Obsidian Vault/trpg-app';
// 本体が大きすぎてローカルLLM（num_ctx=8192）に渡せない目安。.loop/README.md 参照。
...
function renderIndex(all) {
  ...
  out.push('`node .loop/codemap.mjs` で生成される。表の内容を手で直しても次回の生成で消える。');
  out.push('');
  out.push('## 使い方');
  out.push('');
