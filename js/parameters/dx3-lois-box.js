...
// ------------------------------------------------------------------
// 感情表。以前はここに「仮」の一覧を置いていたが、BCDice
// （https://github.com/bcdice/BCDice、BSD-3-Clause）の i18n/DoubleCross/ja_jp.yml
// にある感情表（ET）のポジティブ20件・ネガティブ20件へ差し替えた。出所がはっきりし、
// 中身も正しくなる。並びは1D100の出目順（1-5から96-100）そのまま。
// 著作権表示は NOTICE に置いてある（BSD-3-Clauseは表示の保持を条件にしている）。
//
// 以前の一覧にあって上流に無かった語（傾倒・懐旧・侮蔑・無関心）は落とした。また
// 「意志」は上流では「遺志」なので、そちらに合わせた。
// ここを書き換えても保存済みデータは壊れない：ロイスは感情を「文字列」として持つため、
// リストに無い値（旧リストの値・シート読み込みで入ってきた値）は、その行の選択肢として
// 一時的に追加される（buildEmotionSelect参照）。落とした語も、既に保存されていれば
// これまでどおり表示される。
//
// 末尾の「その他」は表には無い。自由入力へ逃がすためにこのアプリが足している受け皿。
// ------------------------------------------------------------------
