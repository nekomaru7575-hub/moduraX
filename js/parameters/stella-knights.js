...
// アタックダイス補正(DB)に入れておける上限。規則上ダイス追加は1回の判定で3個まで。
const DB_MAX = DICE_ADD_MAX_COUNT;
// 判定のたびにDBぶんを払う処理の、ログでの呼び名。
const DB_CHARGE_LABEL = 'ダイス追加(DB)';
// 直前の判定でDBぶんに払ったブーケの控え。リロールで返す額を決めるためだけに持つ。
// パラメータではなくcomponentsなのは、バフ計算・キャラクター一覧・{名前}記法の
// どれにも出したくない内部の控えだから（Coreはcomponentsの中身を解釈しない）。
const DB_CHARGE_COMPONENT_KEY = 'stellaKnightsAutoDiceAdd';
...
const CHARACTER_PARAMETERS = [
  ...
  //
  // 【手入力にしている理由】自動で書き込む形にすると、{DB}の展開（バフ込みの実効値を読む）に
  // 間に合わせるために「判定を送る前に状態を書き換える」必要が出る。値が最初からここに
  // 在れば、展開は普通にこの値を読むだけで済む。editable:trueは、パネルの入力欄が
  // SET_PARAMETERで書き込むために要る（editable:falseだとガードに弾かれる）。
  { key: 'DB', label: 'アタックダイス補正(DB)', value: 0, visible: false, locked: true, editable: true },
...
// DBに手で入れてある個数（バフは含めない基礎値）。払う額はこの値だけで決める——
// バフ/デバフで動いたぶんまで課金の対象にすると、卓の裁定で乗せた修正にまで金が掛かる。
// 読み方の判断はここ1か所に閉じる。0〜DB_MAXへ丸めるので、取り込んだJSONが何を入れていても
// 1回の判定で払う額は上限を超えない。
function readDiceAddCount(token) {
  const raw = Math.trunc(Number(token?.parameters?.[ATTACK_DICE_BONUS_PARAM_ID]?.value) || 0);
  return Math.min(DB_MAX, Math.max(0, raw));
}

// 直前の判定で払ったブーケの控えを読む。取り込んだJSON（信用しない）が
// 何を入れていても、返すのは0以上の整数（componentsの正規化は各プラグインの責任）。
function readDiceAddRefund(token) {
  const raw = Number(token?.components?.[DB_CHARGE_COMPONENT_KEY]?.cost);
...
function renderStellaKnightsCharacterPanel({
  ...
  if (canWrite) {
    ...
    // DBも同じ。手で入れる値になったので、持っていないと入力欄からの保存が飛ぶ
    if (!readParameters()[ATTACK_DICE_BONUS_PARAM_ID]) {
      dispatch('SET_PARAMETER', { characterId: tokenId, paramId: ATTACK_DICE_BONUS_PARAM_ID, value: 0 });
    }
  }
  ...
  // アタックダイス補正(DB)の入力欄。「今この種別で描いているか」の印も兼ねる
  // （nullなら描いていない＝保存しない）。値の持ち越しは上の pendingValues に任せる
  let dbInput = null;

  function renderTypedArea() {
    rows.forEach(({ paramId, input }) => pendingValues.set(paramId, input.value));
    if (dbInput) pendingValues.set(ATTACK_DICE_BONUS_PARAM_ID, dbInput.value);
    typedArea.innerHTML = '';
    ...
    // --- アタックダイス補正(DB)（手入力 0〜DB_MAX）---
    // この判定で何個ダイスを足すかを、あらかじめここへ入れておく。判定を1回振るたびに
    // 「入っている数×4」のブーケが自動で引かれる（applyStellaKnightsCheckRoll）。
    // 振るダイスの数は式に {DB} と書いて自分で乗せる（DBは自動ではダイス数に足されない）。
    // 能力を持たない種別には出さない＝シースのコマには現れない。
    if (rule.dice) {
      ...
      dbInput.title = `この判定で足すダイスの数（0〜${DB_MAX}）。`
        ...
        + `振るダイスの数は式に {DB} と書いて乗せてください。`;
      dbInput.value = pendingValues.has(ATTACK_DICE_BONUS_PARAM_ID)
        ? pendingValues.get(ATTACK_DICE_BONUS_PARAM_ID)
        : String(readDiceAddCount({ parameters: current }));
      dbInput.disabled = !canEdit;
      bonusRow.appendChild(dbInput);

      // バフ/デバフでも動くので、今の実効値が手入力と違うときだけ添えて出す
      // （js/parameters/dracurouge.jsの目標値修正(TB)と同じ見せ方）
      const token = getToken?.() ?? null;
      const effective = Number(token && getEffectiveParameterValue
        ? getEffectiveParameterValue(token, ATTACK_DICE_BONUS_PARAM_ID)
        : current[ATTACK_DICE_BONUS_PARAM_ID]?.value) || 0;
      const base = readDiceAddCount({ parameters: current });
      if (effective !== base) {
        const note = document.createElement('span');
        note.style.alignSelf = 'center';
        note.style.color = 'var(--text-body-strong)';
        note.style.fontSize = '0.85rem';
        note.textContent = `今の実効値 ${effective > 0 ? '+' : ''}${effective}（バフ/デバフ込み）`;
        bonusRow.appendChild(note);
      }
    } else {
      // 能力を持たない種別へ切り替えたら、入力欄は無かったことにする
 ...
 *
 * 【順番が要】状態を1つも変えないうちに断る理由を全部見る（個数・対象・シース・DB・残高）。
...
function runDiceAdd(input, { token, dispatch, findTokenByName, generateBuffId }) {
  ...
  // DBに手で数が入っているコマへは、その上から足せない。判定のたびにその数ぶんが
  // 自動で引かれるので、重ねると規則上の上限（3個）を超えるため。
  //
  // 【見るのは受け取る側】上限は「誰のダイスが何個増えるか」の話なので、払う側ではなく
  // バフが付くコマで見る。自分のDBに数を入れていても、入れていない味方へ
  // 「ダイス追加(2>味方)」と払ってやることはできる。
  const targetDice = readDiceAddCount(target);
  if (targetDice > 0) {
    alert(`${target.name}のアタックダイス補正(DB)には ${targetDice} が入っています。`
      + `
判定のたびにその数ぶんのブーケが引かれるので、上から足せません。`
      + `
手で足したいときは、キャラクター更新画面でDBを0にしてください。`);
    return true;
  }

  // シースは能力を持たないので、DBのバフも受け取れない
  // （打ったコマのシースはハンドラの頭で断っている）
...
function runReroll(input, { token, dispatch }) {
  ...
  // 直前の判定でDBぶんに払ったものを返す。振り直した先の判定でまた同じだけ取るので、
  // 返さないと1回の判定に二重で払うことになる。
  // 手で打った「ダイス追加」は返さない（自分で決めて払ったものなので、控えも残していない）。
  const refund = readDiceAddRefund(token);
  ...
  if (refund > 0) clearDiceAddRefund(token, dispatch);

  logBouquetSpend(dispatch, token, input, [
    'リロール',
    ...(refund > 0 ? [`${DB_CHARGE_LABEL}を取り消し: ブーケ +${refund}`] : []),
...
/**
 * アタックダイス補正(DB)ぶんの後払い。nSKのアタック判定が1回成立するたび、DBに手で
 * 入れてある個数×4のブーケを払う。Coreが判定の後に呼ぶ
 * （js/parameters/registry.jsのapplyPluginCheckRoll）。
 *
 * 【振る値には触らない】DBは手入力の値として判定より前から状態に在り、{DB}の展開が
 * それを読んでBCDiceへ届けている。ここは「振れたぶんの代金を後から取る」だけで、
 * パラメータもバフも動かさない。
 *
 * 【マイナスを許す】払うと決まるのは振り終わった後なので、残高が足りなくても引き返せない。
 * 止めずに引き、マイナスになったことをその1行に書く。卓で清算してもらう。
 *
 * 【払うのは基礎値だけ】バフ/デバフで動いたDBは対象にしない（readDiceAddCount）。
 * 卓の裁定や「ダイス追加(n)」コマンドで乗った修正にまで金を取らないため。
 *
 * @returns {{ logText: string }|null} 判定のログへ添える1行（何もしなければnull）
 */
function applyStellaKnightsCheckRoll({ command, token, dispatch }) {
  if (!token || !looksLikeStellaKnightsAttack(command)) return null;
  if (!rulesOf(token).dice) return null; // シース（DBの欄も出さないが、念のため）

  const count = readDiceAddCount(token);

  // 【控えの寿命】払わなかった判定を通ったら、前の判定の控えは捨てる。残したままにすると、
  // ずっと前の判定のぶんをリロールが返してしまう（返すのは「直前の判定」だけ）。
  if (count === 0) {
    clearDiceAddRefund(token, dispatch);
    return null;
  }

  const cost = count * DICE_ADD_COST_PER_DIE;
  const before = Number(token.parameters?.[BOUQUET_PARAM_ID]?.value) || 0;
  const after = before - cost;

  dispatch('SET_PARAMETER', {
    characterId: token.id, paramId: BOUQUET_PARAM_ID, value: after
  });

  // リロールで返す額を控える（次の判定が上書きする）
  dispatch('SET_COMPONENT', {
    id: token.id, componentKey: DB_CHARGE_COMPONENT_KEY, value: { cost }
  });

  // 「＞」は使わない（最後の「＞」の後ろを最終値として読む処理がある。js/main.jsのparseFinalDiceNumber）
  return {
    logText: `
${DB_CHARGE_LABEL}: ${count}個 ／ ブーケ -${cost}（${before} → ${after}）`
      + (after < 0 ? '　⚠ ブーケが足りていません' : '')
  };
}

// 控えを消す。既に無ければ何もしない（無駄なdispatchを全員へ配らない）。
function clearDiceAddRefund(token, dispatch) {
  if (readDiceAddRefund(token) === 0) return;
  dispatch('SET_COMPONENT', {
    id: token.id, componentKey: DB_CHARGE_COMPONENT_KEY, value: null
  });
}
