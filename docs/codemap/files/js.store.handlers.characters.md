---
source: js/store/handlers/characters.js
lines: 331
exports: 1
imported_by: 1
api_sha: 7fef7672ac8c
prose_sha: 7fef7672ac8c
generated: 2026-09-16
tags: [codemap]
---

# js/store/handlers/characters.js

<!-- prose:summary -->
コマそのものの作成・削除と、コマが持つパラメータの操作。
<!-- /prose:summary -->

## 役割

<!-- prose:role -->
_(未記入)_
<!-- /prose:role -->

## export（1）

| 行 | 種別 | 名前 | シグネチャ | 説明 |
|---:|---|---|---|---|
| 23 | const | CHARACTERS_HANDLERS | `CHARACTERS_HANDLERS` |  |

## トップレベル関数（LOCAL TASKS 候補）（0）

なし（`function 名(...) {}` 宣言がトップレベルに無い）。

## 依存

- import → [[js.EventBus]], [[js.parameters.core]], [[js.parameters.registry]], [[js.store.images]], [[js.store.params]], [[js.store.patch]], [[js.store.room]], [[js.store.round-state]], [[js.store.targets]]
- imported by → [[js.store.handlers.index]]

## 注意

<!-- prose:notes -->
_(未記入)_
<!-- /prose:notes -->
