...
| `steps` | 1手番／1フェーズを何回の押下で進めるかの宣言。`{ id, label, onlyWhen }[]`（[3.6.2](#362-手番を刻む-steps)） |
...
既定では、キャラクターの手番は主ボタン1回（「手番を終了」）で終わる。進行役が卓へ
「今これをやります」と宣言しながら刻みたいシステムでは、フェーズに `steps` を宣言する。

```js
{
  id: 'action', label: 'アクション', kind: 'perCharacter',
  steps: [
    { id: 'omen',    label: '予兆を開示',   onlyWhen: { paramId: 'SK:turnOrder', value: 2 } },
    { id: 'turnEnd', label: '手番終了' },
    { id: 'routine', label: 'ルーチン発動', onlyWhen: { paramId: 'SK:turnOrder', value: 2 } }
  ]
}
```

**押すたびに1段進み、最後の段を押すとこれまでどおり完了する**（手番なら次の手番へ、
フェーズなら次のフェーズへ）。押すたびに `applyRoundEvent` へ `type: 'step'` が飛ぶので、
何を出すかは拡張ルーム設定（[3.13](#313-拡張ルーム設定roomextensions)）が決める。
ボタンのラベルは宣言した `label` がそのまま出る（`kind: 'once'` のフェーズにも宣言できる）。

`onlyWhen` は `skipWhen` と同じ `{ paramId, value }` の比べ方で、**手番のコマの実効値が
一致する段だけ含める**。手番を持たないフェーズでは無視して常に含める。

**最後の「当てはまる」段が完了させる**ので、1つの宣言でコマごとに押す回数を変えられる。
ステラナイツのアクションの段は5つの段を宣言していて、`onlyWhen` の付いていない段が
「手番終了」の1つだけ。だからブリンガーは5回、NPCは1回押して手番が終わる。

> **段は添字ではなく id で進む**（`round.step` が `'preTurn' | 'act' | <段のid>` になる）。
> 手番中のコマを参加者から外すと当てはまる段が減るので、添字だと範囲外を読んで進行が
> 止まってしまう。id なら、一覧に無い id は「次の1押しで完了」へ自然に落ちる。

押しすぎたときは、ラウンドパネルの「⋯」→「段を1つ戻す」でGMが1つ戻せる。戻るのは
**次に押す段だけ**で、既に流れた発言もプラグイン側の進行も動かない。

ラベルは進行ボタンの文字になり、`round.template` は**取り込んだ部屋のJSONからも来る**ので、
Core 側で件数（8）とラベルの長さ（20字）を丸めている。
...
#### プールを振らずに動かす（`dice.change` / `dice.add` / `dice.erase`）

`diceDraft` を宣言すると、**プラグイン側に1行も書かずに**次の3つのコマンドが生える
...
dice.add(6*3)         3個足す
dice.erase(3)         プールにある3の目を1個廃棄する
dice.erase(3*2)       2個廃棄する。そろっていなければ1個も消さない
```

触るのは**プールだけ**。スキルの下に乗っているダイスは対象外で、`kind: 'match'` の
「乗っている目＝対応する数字」という不変条件は崩れない。`dice.erase` も同じで、
コマンドは目でしか指せない＝乗っている分まで巻き込むと「どれが消えるか」が打つ前に見えない。
乗っているダイスを捨てるのは、パネルのゴミ箱（どこにあっても1個ずつ落とせる）の仕事。
...
**同じ操作を自分のコマンドへ組み込む**なら `runDiceChange()` / `runDiceAdd()` /
`runDiceErase()` を呼ぶ（id を指定して消すなら `runDiceDiscard()`。パネルのゴミ箱もこれを通る）。
...
**廃棄は黙って消さない。** `runDiceDiscard` は `silent` を指定しない限りチャットへ1行出す。
取り消せないうえ、他の人から見るとダイスが理由なく減るため（振って溢れた分を必ず伝えるのと
同じ作法）。文言は**消えたダイスそのもの**から書き起こすので、コマンドで消してもゴミ箱へ
落としても同じ物には同じ1行が出る。
...
```js
applyRoundEvent: (value, event) => ({ value, entries, logText })
// event.type  … 'phaseStart'（段に入った） | 'turnStart'（手番が決まった）
//               | 'step'（フェーズが宣言した段を1つ押した。下記）
//               | 'progressionEnd'（ラウンド進行そのものが終わった）
// event.phase … 今の段（テンプレートの1件そのまま）。progressionEnd では null
// event.actor … 手番のコマ。手番のあるフェーズでだけ入り、それ以外は null
// event.stepId … 'step' のときだけ入り、それ以外は null
// event.roundNumber … 何ラウンド目か
```

何もしないなら `null`、値が変わらないなら**同じ参照**の `value` を返す。
発言の並びは「押されたぶん（`step`）→ 進行の知らせ → `phaseStart` と `turnStart` のぶん」。
押した結果が、次に起きることの予告より先に出る。

戻せる発言は2通り。`entries` は表示名を自分で決める発言（`[予兆]` `[舞台]`）、
`logText` は**「システム」の1行に混ざる**もの。Coreの知らせと並べたいときは `logText`。

> **手番の終わりは `step` で受ける。** [`steps`](#手番を刻む-steps) の最後の段が手番の
> 終わりにあたるので、`turnEnd` のような別のイベントは持たない（二重に飛ぶため）。
> `phaseStart` / `turnStart` / `progressionEnd` が残っているのは、**押下なしで発火する**
> ＝`steps` では表せないから。

> **「戦闘が終わったので最初へ戻す」は `progressionEnd` で受けること。**
> `resetOnPhaseEnd` は「毎ラウンドのカット」と「進行の終了」の**両方から同じ `'round'` で**
> 呼ばれるので、2つを区別できない。あちらでやると毎ラウンド巻き戻る。

#### 手番の知らせは、プラグインが出したらCoreは出さない

`turnStart` で `logText` を返すと、Coreは自分の「○○の手番です。」も、段に入った行の
「（手番: ○○）」も**出さない**（同じことを2回言わないため）。つまりその段の手番の知らせは
プラグインが引き取ることになるので、**知らせたいコマ全員ぶんを返すこと**
（ステラナイツはブリンガーに「○○の手番。予兆を公開します」、NPCに「○○の手番。」を返す）。

拡張の値がまだ無い部屋には `applyRoundEvent` 自体が届かないので、そこでは今までどおり
Coreの文言が出る。
...
チャット欄へ判定が打たれるたび、**BCDice へ送る前に**適用中のシステムへ知らせる。
...
**ここで付けたバフは、その判定に乗る。** `{パラメータ名}` の展開はバフ込みの実効値を読むので、
この口で `ADD_BUFF` すればダイス数へ届く（`{DB}+8SK4` と書けば `11SK4` になる）。
展開の後では今回の判定に乗らないため、この口は「判定が済んだ後の後始末」ではなく
**「これから振るための前払い」**に使う。
...
- 呼ばれるのは、参照キャラクターを選んで打たれた入力すべて。**まだ BCDice へ送る前なので、
  構文を認識できない入力や、この後のコマンド解釈が食う入力でも呼ばれてしまう。**
  課金を伴う副作用を置くなら、発動条件を自分の書式の完全一致まで絞ること
  （そうしないと「振っていないのに払った」が起きる）
- 拡張判定UIの「振る」は `DICE_ROLL_REQUESTED` を通らないので呼ばれない
- ここで付けた `expirePhase: 'check'` のバフは、**その判定で消える**（判定の後に Core が
  `EXPIRE_BUFFS` を撃つ）。次の判定へは持ち越さない
- 返した1行は、その判定のログへ添えられる。**平文の発言に落ちた場合でも必ず出る**
  （払ったのにどこにも残らない、を作らないため）
- 走るのは**振った本人の画面だけ**。reducer ではないので全員で二重には走らない。
  中で乱数や時刻に依存した分岐を書かないこと（同期が崩れる）

ステラナイツが、nSK のアタック判定のたびに「アタックダイス補正(DB)に入っている数×4」の
ブーケを払うのに使っている。DB は手入力の値で判定より前から状態に在るので、この口は
**払うだけ**で振る値には触らない。
