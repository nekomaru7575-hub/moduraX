...
//
// 【SVGを置けない場所の書き方】
// <option>の中には要素を置けない（HTMLの仕様）ので、部屋一覧（js/room-index.js）には
// 一時期🔒をそのまま置いていた。今は絵文字ではなく「パスワードあり」と文字で書いている。
// SVGに揃えられない場所では、絵文字で代用するのではなく文字で言うこと。
// 字面がOS任せになる問題も、読み上げ機が「錠」と読む問題も、それで消える。
